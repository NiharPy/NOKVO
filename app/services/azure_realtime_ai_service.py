"""
Azure OpenAI realtime-mini provisioner for Nokvo One.
...
If AZURE_SUBSCRIPTION_ID is unset, returns a 'skipped' status so dev/test environments
can still run the orchestrator without Azure credentials.
"""
...
class AzureRealtimeAIService:
    ...
    def _purge_soft_deleted_account(client: CognitiveServicesManagementClient, region: str, account_name: str) -> bool:
        ...
        try:
            ...
            # Older SDK signature: (location, account_name) only
            try:
    ...
    @staticmethod
    async def provision_realtime_account(
        rg_name: str,
        tenant_id: str,
        slug: str,
        organization_name: str,
    ) -> dict:
        region = settings.AZURE_OPENAI_REALTIME_REGION or "swedencentral"
        account_name = AzureRealtimeAIService._account_name(slug, tenant_id)

        if not settings.AZURE_SUBSCRIPTION_ID:
            return {
                "account_name": account_name,
                "deployment_name": settings.AZURE_OPENAI_REALTIME_DEPLOYMENT,
                "model": settings.AZURE_OPENAI_REALTIME_MODEL,
                "model_version": settings.AZURE_OPENAI_REALTIME_MODEL_VERSION,
                "region": region,
                "endpoint": None,
        ...
        def _create_account() -> Any:
            return client.accounts.begin_create(
                ...
                account_name=account_name,
                account=AzureRealtimeAIService._build_account_payload(region, tenant_id, account_name),
            ).result()
        ...
        try:
            ...
            if error_code == "FlagMustBeSetForRestore":
                ...
                AzureRealtimeAIService._purge_soft_deleted_account(client, region, account_name)
                try:
                    account = _create_account()
        ...
        try:
            deployment_payload = Deployment(
                sku=Sku(name=settings.AZURE_OPENAI_REALTIME_SKU, capacity=1),
                properties=DeploymentProperties(
                    model=DeploymentModel(
                        format="OpenAI",
                        name=settings.AZURE_OPENAI_REALTIME_MODEL,
                        version=settings.AZURE_OPENAI_REALTIME_MODEL_VERSION,
                    ),
            ...
            raise RuntimeError(f"Failed to provision Azure OpenAI realtime mini: {exc}") from exc

    @staticmethod
    async def delete_realtime_account(rg_name: str, account_name: str) -> bool:
