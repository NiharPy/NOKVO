...
class IntentParser:
    ...
    def parse(prompt: str, integration_type: str, provider: str, context: dict[str, Any]) -> ToolIntent:
        ...
        if integration_type == "database" and classification.operation_type in {"create", "update", "workflow", "delete"}:
        ...
        return ToolIntent(
            ...
            business_preconditions=IntentParser._preconditions(classification.normalized_prompt, operation_type, classification.workflow_type, target_field),
            ...
            intent_signals=IntentClassifier.to_dict(classification),
    ...
    @staticmethod
    def _preconditions(text: str, operation_type: str, workflow_type: str | None, target_field: str | None) -> list[str]:
    ...
    def _resolved_username_column(context: dict[str, Any], schema_context: dict[str, Any], target_resources: list[str]) -> str | None:
        username_column = IntentParser._first_verified_column(schema_context, target_resources, ("username", "user_name"))
        if username_column:
        ...
        if confirmed == "full_name":
            full_name_column = IntentParser._first_verified_column(schema_context, target_resources, ("full_name",))
            if full_name_column:
                return full_name_column
        return None

    @staticmethod
    def _first_verified_column(schema_context: dict[str, Any], target_resources: list[str], field_names: tuple[str, ...]) -> str | None:
        for table in schema_context.get("tables", []):
            if table["fqn"] not in target_resources:
                continue
            for column in table.get("columns", []):
