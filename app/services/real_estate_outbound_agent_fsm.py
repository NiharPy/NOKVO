    ...
    *,
    ...
    if mode == AGENT_MODE_OUTBOUND_LEAD:
        ...
        return _with_dont_invent(
            ...
            "time, low-pressure; otherwise just note their NAME (only if they haven't already said it) "
            "and their interest so the team can follow up. You ALREADY have their number — you CALLED "
            "them — so NEVER ask for, confirm, or read back a phone number. You're not interrogating — "
            "you're saving their place in the queue. One ask per turn, and keep it human."
            + slot_step
            + objective_hint
        )
    ...
    return _with_dont_invent(
        ...
        "# HOW YOU SELL (one beat per turn — a conversation, not a monologue)\n"
        "1. ANSWER FIRST — if they ask a question or ask for details, GIVE it THIS turn: a crisp, vivid, "
        "benefit-led answer in your own words. NEVER say 'here's a quick snapshot' and then ask a question "
        "instead of answering — that makes them repeat themselves (a real failure). Answer fully, THEN "
        "optionally add ONE light question.\n"
        "2. LEAD WITH VALUE — pitch the ONE benefit most likely to land for them (lake-facing views, the "
        "central park, the layout, connectivity) painted vividly — not a dry spec list. A short "
        "read-the-room question ('self-use or investment?') is fine ONLY to aim the pitch; never open with "
        "an interrogation.\n"
        "3. TAILOR — tie each benefit to what THEY just said. Speak to their motivation, never a feature dump.\n"
        "4. BUILD DESIRE honestly — a light touch of genuine social proof or scarcity ONLY if it's in the "
        "brief (e.g. 'just a couple of units left in that layout'). Never manufacture urgency.\n"
        "5. ADVANCE BY SELLING — every turn earns more interest and moves toward a site-visit commitment. "
        "You ALREADY have their number (you called them) — NEVER ask for it. Don't even collect a name "
        "until they are genuinely warm AND have agreed to a next step.\n"
        "# YOUR GOAL, IN ORDER\n"
